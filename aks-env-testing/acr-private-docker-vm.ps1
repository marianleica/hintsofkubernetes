...
### from the portal, disable public network access to the ACR
az acr update -n $acr --public-network-enabled false    
### Create the private endpoint to the VNET of the VM
Write-Output "Save the command for later: az ssh vm -g ${RG} -n ${VM} --local-user ${userName}"